...
- (void)beginRequestForAccessToken
{
    ...
    NSString *url = [NSString stringWithFormat:@"%@?client_id=%@&redirect_uri=%@&response_type=token&display=mobile",self.autherizeURL,self.appKey,[self.callbackURL URLEncodedString ]];
    authorizationController.authorizationURL= url;
    authorizationController.authorizationDelegate=self;
    OUONavigationController *navController=[[OUONavigationController alloc] initWithRootViewController:authorizationController];
